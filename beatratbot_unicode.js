...
function leftToMonday() {
	var d = new Date();
	return (-d + d.setHours(12, 0, 0, 0) + ((8 - d.getDay()) % 7) * 24 * 60 * 60 * 1000);
}

function mondays(bot) {
	var monday = "./resources/monday.jpg";
	// bot.channels.get("268209621724823554").send("Time until next Monday: " + Math.floor(leftToMonday()/1000/60/60/24) + " days " + Math.floor(leftToMonday()/1000/60/60%24) + " hours " + Math.floor(leftToMonday()/1000/60%60) + " minutes.");
...
function DQueries(bot, message, q) {
    ...
    // direct PMs
		...
		if (userNames.indexOf(dmName) != -1) {
			userObjects[userNames.indexOf(dmName)].send(dmMsg);
		} else {
